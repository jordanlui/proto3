/* Outputting IMU and Quaternion data (via non-magnetometer Madgwick AHRS algorithm)
 *  Output is a 23 byte word which contains acc, gyro, quaternion, microsecond time stamp and and end character.
 *  Note that the code below is very messy and needs tidying
 *  2018 May 3. Code working but messy. 23 byte word output which can be saved in existing LabVIEW recording setup
 */
...
//      pitch *= 180.0f / PI; // Convert values to degrees from radians
//      yaw   *= 180.0f / PI; 
//      roll  *= 180.0f / PI;
//        
...
//      Serial.print("average rate = "); Serial.print(1.0f/deltat, 2); Serial.println(" Hz");
      ...
      // Format a byte string and send in format to match LabVIEW. Transmission should be 23 bytes
      // Note the arbitrary scaling listed below allows us to send values as two byte unsigned int and recover in Labview
      acceldata[0] =  (ax) * 100 + 800;
      acceldata[1] =  (ay) * 100 + 800;
      acceldata[2] =  (az) * 100 + 800;
      gyrodata[0] =  (gx) * 10 + 32500;
      gyrodata[1] =  (gy) * 10 + 32500;
      gyrodata[2] =  (gz) * 10 + 32500;
      quatshift[0] = q[0] * 100 + 800;
      quatshift[1] = q[1] * 100 + 800;
      quatshift[2] = q[2] * 100 + 800;
      quatshift[3] = q[3] * 100 + 800;
...
//      getbytes();
      for (int i = 0; i < 3; i++)
      {
        ...
        if(accelhighbyte[i]==0xA)
          ...
          if(accelhighbyte[i]==0xA)
          {      accellowbyte[i]=0xB;    }
        Serial.write(accelhighbyte[i]);
        Serial.write(accellowbyte[i]);
      }
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        if(gyrohighbyte[i]==0xA)
        {      gyrolowbyte[i]=0xB;    }
        Serial.write(gyrohighbyte[i]);
        Serial.write(gyrolowbyte[i]);
      }
      ...
      for (int i = 0; i < 4; i++)
      {
        ...
        if(quathighbyte[i]==0xA)
        {      quatlowbyte[i]=0xB;    }
        Serial.write(quathighbyte[i]);
        Serial.write(quatlowbyte[i]);
      }
      deltatime = millis();
      Serial.write(highByte(deltatime));
      Serial.write(lowByte(deltatime));
      Serial.write('\n');
    

  } // End of selective timing transmission loop
}
...
void getbytes()
// Get the high and low bytes for acc, gyro, quaternion
// Acc, gyro, and quaternion data should be 2 byte int
// Omron should be 1 byte int
{

//  for (i = 0; i < 16; i++)
//  {
//    if(tdatashift[i]==0xA)    {      tdatashift[i]=0xB;    }
//  }
